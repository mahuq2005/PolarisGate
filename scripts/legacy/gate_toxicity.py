#!/usr/bin/env python3
"""Toxicity Detection Gate — runs inside Docker via `docker compose run`.
Prints F1 score on stdout for run_gates.sh to parse.
Threshold check is done by the caller.
"""
TOXIC_KW = [
    "idiot", "useless", "hurt", "kill", "worthless", "trash", "stupid",
    "hate", "scammer", "liar", "die", "dumb", "moron", "disgusting",
    "terrible", "failure",
]
...
def classify(text: str):
    t = text.lower()
    m = sum(1 for kw in TOXIC_KW if kw in t)
    s = min(1.0, m * 0.25)
    return s >= 0.5, s
